...
class FbBotController extends Controller
{
    ...
    public function getJadwalKuliah($userId) {
      ...
      if($semuaJadwal->count() > 0) {
        ...
        $textSenin = "--===Senin===--" . PHP_EOL . $senin;
        $this->setSendCondition($userId, $textSenin);
        $textSelasa = "--===Selasa===--" . PHP_EOL . $selasa;
        $this->setSendCondition($userId, $textSelasa);
        $textRabu = "--===Rabu===--" . PHP_EOL . $rabu;
        $this->setSendCondition($userId, $textRabu);
        $textKamis = "--===Kamis===--" . PHP_EOL . $kamis;
        $this->setSendCondition($userId, $textKamis);
        $textJumat = "--===Jumat===--" . PHP_EOL . $jumat;
        $this->setSendCondition($userId, $textJumat);


      } else {
        $textSend = "Maaf anda belum memasukkan data jadwal kuliah.";
        $this->setSendCondition($userId, $textSend);
      }
    }
